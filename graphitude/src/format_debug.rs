    ...
    E: Debug,
{
    ...
    fmt.debug_struct(name)
        ...
        .field(
            "edges",
            &FormatDebugWith(|f: &mut Formatter<'_>| {
                let make_edge_tag = |eid: &EdgeId<G>| {
                    let (n1, n2) = eid.ends();
...
mod tests {
    ...
    fn test_format_debug() {
        let mut graph = Graph::<LinkedGraph<&str, i32, Directed, MultipleEdges>>::default();
        ...
        let expected = r#"Graph { nodes: {0: "A", 1: "B"}, edges: {0 -> 1: 10} }"#;
    ...
    fn test_format_debug_with_undirected() {
        ...
        let mut graph = Graph::<UndirectedGraph>::default();
        ...
        // Check structure
        assert!(output.starts_with("Graph { nodes: {"));
